...
- **Key changes:**
  ...
  - Added MCP unified pipeline integration artifacts (baseline tied to world_class_validation): README (`workspace/mcp/README.md`), pipeline manifest (`workspace/mcp/pipelines/unified-pipeline-config.yaml`), JSON Schema (`workspace/mcp/schemas/unified-pipeline.schema.json`), TypeScript types (`workspace/mcp/types/unifiedPipeline.ts`), and Python loader (`workspace/mcp/tools/load_unified_pipeline.py`).
